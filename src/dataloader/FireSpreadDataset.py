...
from .utils import get_indices_of_degree_features, get_means_stds_missing_values
...
class FireSpreadDataset(Dataset):
    def __init__(
        ...
        self.n_leading_observations_test_adjustment = (
            n_leading_observations_test_adjustment
        )
    ...
    def find_image_index_from_dataset_index(self, target_id) -> (int, str, int):
        ...
        for fire_year in self.datapoints_per_fire:
            if found_fire_year is None:
                for fire_name, datapoints_in_fire in self.datapoints_per_fire[
                    fire_year
                ].items():
    ...
    def load_imgs(self, found_fire_year, found_fire_name, in_fire_index):
        ...
        if self.load_from_hdf5:
            ...
            imgs_to_load = self.imgs_per_fire[found_fire_year][found_fire_name][
                in_fire_index:end_index
            ]
            imgs = []
    ...
    def __getitem__(self, index):
        ...
        found_fire_year, found_fire_name, in_fire_index = (
            self.find_image_index_from_dataset_index(index)
        )
    ...
    def preprocess_and_augment(self, x, y):
        ...
        landcover_encoding = (
            self.one_hot_matrix[landcover_classes_flattened]
            .reshape(new_shape)
            .permute(0, 3, 1, 2)
        )
        x = torch.concatenate(
    ...
    def flatten_and_remove_duplicate_features_(self, x):
        ...
        Returns:
            _type_: _description_
        """
        static_feature_ids, dynamic_feature_ids = (
            self.get_static_and_dynamic_features_to_keep(self.features_to_keep)
        )
        dynamic_feature_ids = torch.tensor(dynamic_feature_ids).int()

        x_dynamic_only = x[:-1, dynamic_feature_ids, :, :].flatten(
    ...
    def get_static_and_dynamic_features_to_keep(features_to_keep: Optional[List[int]]):
        ...
        if type(features_to_keep) == list:
            dynamic_features_to_keep = list(
                set(dynamic_features_to_keep) & set(features_to_keep)
            )
            dynamic_features_to_keep.sort()

        if type(features_to_keep) == list:
            static_features_to_keep = list(
                set(static_features_to_keep) & set(features_to_keep)
            )
    ...
    def img_dates_to_doys(img_dates):
        ...
        return [
            datetime.strptime(img_date.replace(".tif", ""), date_format)
            .timetuple()
            .tm_yday

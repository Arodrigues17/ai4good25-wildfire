...
from src.models import SMPModel, ConvLSTMLightning, LogisticRegression, UTAELightning, TransformerCA  # type: ignore
...
def map_model_class(name: str):
    ...
    if name in ("transformercamodule", "transformerca"):
        return TransformerCA
    raise ValueError(
        f"Unknown model '{name}'. Choose from: smp, convlstm, logisticregression, utae, transformercamodule"
    )
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            if use_doy:
                ...
                y = y.to(device)
                doys = doys.to(device)
                y_hat, y_out, _ = model.get_pred_and_gt((x, y, doys))
            else:
                x, y = batch
                x = x.to(device)
                y = y.to(device)
                y_hat, y_out, _ = model.get_pred_and_gt((x, y))
